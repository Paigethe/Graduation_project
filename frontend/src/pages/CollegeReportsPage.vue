...
const mockPreviewMetrics = computed(() => (reportMetrics.value as ReportMetric[]).filter((metric) => ALL_REPORT_METRICS.includes(metric)))
const mockPreviewPeriod = computed(() => {
  if (Array.isArray(reportRange.value) && reportRange.value.length === 2 && reportRange.value[0] && reportRange.value[1]) {
    return `${reportRange.value[0]} 至 ${reportRange.value[1]}`
  }
  return '2026-03 至 2026-04'
})
const mockPreview = computed<ReportSummary>(() => ({
  metrics: mockPreviewMetrics.value,
  period: {
    label: mockPreviewPeriod.value,
    month_start: Array.isArray(reportRange.value) ? reportRange.value[0] : '2026-03',
    month_end: Array.isArray(reportRange.value) ? reportRange.value[1] : '2026-04',
  },
  assessments: {
    count: 128,
    avg_score: 3.18,
    ...
    top_dimensions: [
      { key: 'sleep', name: '睡眠', avg_score: 3.92, sample_count: 128 },
      { key: 'anxiety', name: '焦虑', avg_score: 3.68, sample_count: 128 },
      { key: 'depression', name: '抑郁', avg_score: 3.41, sample_count: 128 },
    ],
  },
  alerts: {
    count: 34,
    ...
    high_unack_count: 6,
    trend: {
      previous_period_label: '2026-01 至 2026-02',
      alert_change_pct: 13.33,
      high_unack_change_pct: -14.29,
    },
  },
  interventions: {
    count: 29,
    intervention_rate: 64.29,
    counselor_involved_rate: 53.57,
    high_risk_student_count: 28,
    intervened_high_risk_student_count: 18,
    feedback_status_distribution: {
      draft: 2,
      sent: 5,
      in_progress: 9,
  ...
  retest: {
    ...
    baseline_high_risk_count: 12,
    improved_count: 7,
    improved_to_safe_count: 5,
    improvement_rate: 41.67,
  },
  comparisons: {
    previous_period_label: '2026-01 至 2026-02',
    assessment_change_pct: 9.4,
    high_risk_change_pct: -6.25,
  },
}))
